...
#include "explore/wavefront_frontier_detection.hpp"
#include "sensor_msgs/PointCloud.h"
#include "move_base_msgs/MoveBaseAction.h"
#include "explore/FFD.hpp"
...
public:
	...
	void mapCallback( const nav_msgs::OccupancyGrid& map )
	{
		...
		int num_points = 0;
		// std::cout << " 1frontiers.size() " << frontiers.size() << std::endl;
		// std::cout << "1frontiers[0].size() " << frontiers[0].size() << std::endl;
		...
		int pointI = 0;
		// std::cout << "2frontiers.size() " << frontiers.size() << std::endl;
		// std::cout << "2frontiers[0].size() " << frontiers[0].size() << std::endl;
		for(int i = 0; i < frontiers.size(); i++) {
			// std::cout << "frontiers[i].size() " << frontiers[i].size() << std::endl;
			for(int j = 0; j < frontiers[i].size(); j++) {
