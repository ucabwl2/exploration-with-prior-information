            ...
            if (use_graph_) 
            {
              if (graph->wasInit()) {
                ...
                if (do_pub_ndtmap_marker_)
                {
				  ...
				  perception_oru::NDTMap* map_moved = graph->getLastFeatureFuser()->map->pseudoTransformNDTMap(graph->getT());
...
// 							perception_oru::toMessage(graph->getMap(), map_msg, sstm.str() );
...
// 					exit(0);
					...
					for(auto it = cells.begin() ; it != cells.end() ; ++it){
						...
						perception_oru::NDTCell* celltest = NULL;
						pcl::PointXYZ pt;
						pt.x = mean.x;
						pt.y = mean.y;
						pt.z = mean.z;
						...
						auto lazy = dynamic_cast<perception_oru::LazyGrid*>(graph->getMap()->getMyIndex());
					...
					// exit(0);
					perception_oru::toOccupancyGrid(graph->getMap(), omap, occ_map_resolution_, world_frame);
...
// 					pt.y = mean1.y;
// 					pt.z = mean1.z;
// 					auto lazyt = dynamic_cast<perception_oru::LazyGrid*>(graph->getMap()->getMyIndex());
// 					int indX,indY,indZ;
...
// // 					map_publisher_.publish(map_msg);
              ...
              if (fuser->wasInit()) {
                ...
                if (do_pub_occ_map_) {
                  nav_msgs::OccupancyGrid omap;
                  perception_oru::toOccupancyGrid(fuser->map, omap, occ_map_resolution_, world_frame);
                  map_pub_.publish(omap);
                }
              }
            }
...
// 			est_file_ << frameTime << " " << perception_oru::transformToEvalString(pose_);
// 		}
...
// 				ndt_map::NDTMapMsg mapmsg;
// 				_gvisu.printAll(*graph, origins, origins_odom, mapmsg, "/world");'
//	            perception_oru::toMessage(graph->getMap(), mapmsg, world_frame);
// 				_marker_pub_graph.publish(origins);
// 				_marker_pub_graph_odom.publish(origins_odom);
// 				_last_ndtmap.publish(mapmsg);
...
// 			Tm.setIdentity();
		...
		if (gt_file_.is_open()) {
			//              gt_file_ << frame_time << " " << perception_oru::transformToEvalString(Tgt);
...
// 		CALLGRIND_STOP_INSTRUMENTATION;
	...
	void gt_callback(const nav_msgs::Odometry::ConstPtr& msg_in)
	{
	    ...
	    gt_pose = Eigen::Translation3d (msg_in->pose.pose.position.x,
            ...
            // if (gt_file_.is_open()) {
            //   gt_file_ << msg_in->header.stamp << " " << perception_oru::transformToEvalString(gt_pose);
