...
inline Eigen::Transform<double,3,Eigen::Affine,Eigen::ColMajor> ICPwithCorrMatch(perception_oru::NDTMap &targetNDT, perception_oru::NDTMap &sourceNDT, const std::vector<std::pair<int,int> > &corresp) {
...
inline void addNDTCellToMap(perception_oru::NDTMap* map, perception_oru::NDTCell* cell) {
  perception_oru::CellVector* idx = dynamic_cast<perception_oru::CellVector*> (map->getMyIndex());
  if (idx != NULL) {
...
inline void setNDTCellToMap(perception_oru::NDTMap *map, perception_oru::NDTCell* cell) {
  perception_oru::LazyGrid* lz = dynamic_cast<perception_oru::LazyGrid*> (map->getMyIndex());
  if (lz != NULL) {
    perception_oru::NDTCell* nd = (perception_oru::NDTCell*)cell->clone();
    // Is there already a cell here?
    perception_oru::NDTCell* c = NULL;
...
inline bool discardCell(perception_oru::NDTMap &map, const pcl::PointXYZ &pt) {
