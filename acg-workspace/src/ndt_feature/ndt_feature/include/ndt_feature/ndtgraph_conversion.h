...
// 		std::cout << "Nb of init when creating message (more than 0) " << fuser.map->getAllInitializedCells().size() << std::endl;
		bool good = perception_oru::toMessage(fuser.map, m.map, "/world");
		m.ctr = fuser.ctr;
...
// 		std::cout << "GETTING THE NODE FUCKER TO WORK gosh" << node.map->Tnow.matrix() << std::endl;
		...
		fuserHMTToMsg(node.getFuser(), m.fuser);
...
// 		perception_oru::NDTMap* map = new perception_oru::NDTMap(new perception_oru::LazyGrid(fuser.params_.resolution));
...
// 			std::cout << "No deleting the fuser's map" << std::endl;
		...
		bool good = perception_oru::fromMessage(lz, fuser.map, m.map, frame, true);
...
// 		std::cout << "HMT convertion" << std::endl;
		msgTofuserHMT(m.fuser, node.getFuser(), frame);
