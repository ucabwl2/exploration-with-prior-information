...
public:
  ...
  perception_oru::NDTMap *map;  /// The complete NDT map (will be updated when optimize is called)
  ...
  perception_oru::NDTMap* getMap() {
    return nodes_.back().map->map;
  }
  
  perception_oru::NDTMap* getMap(int i) {
    return nodes_[i].map->map;
  }
  
  const perception_oru::NDTMap* getMap(int i) const {
    return nodes_[i].map->map;
  }
