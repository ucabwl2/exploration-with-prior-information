...
inline double lineSearchMTFusionTcov(    Eigen::Matrix<double,6,1> &increment,
                                  std::vector<perception_oru::NDTCell*> &sourceNDT,
                                  perception_oru::NDTMap &targetNDT,
                                  std::vector<perception_oru::NDTCell*> &sourceNDT_feat,
                                  perception_oru::NDTMap &targetNDT_feat,
                                  perception_oru::NDTMatcherD2D &matcher_d2d,
                                  perception_oru::NDTMatcherFeatureD2D &matcher_feat_d2d,
                                  ...
                                  const Eigen::MatrixXd &C)
{
    ...
    std::vector<perception_oru::NDTCell*> sourceNDTHere, sourceNDTHere_feat;
    ...
    while (1)
    {
        ...
        if (brackt)
        {
            stmin = perception_oru::NDTMatcherD2D::MoreThuente::min(stx, sty);
            stmax = perception_oru::NDTMatcherD2D::MoreThuente::max(stx, sty);
        }
        else
        {
            stmin = stx;
        ...
        stp = perception_oru::NDTMatcherD2D::MoreThuente::max(stp, stpmin);
        stp = perception_oru::NDTMatcherD2D::MoreThuente::min(stp, stpmax);
        ...
        {
            perception_oru::NDTCell *cell = sourceNDT[i];
            if(cell!=NULL)
            {
                ...
                perception_oru::NDTCell* nd = (perception_oru::NDTCell*)cell->copy();
                nd->setMean(mean);
                nd->setCov(cov);
        ...
        {
            perception_oru::NDTCell *cell = sourceNDT_feat[i];
            if(cell!=NULL)
            {
                Eigen::Vector3d mean = cell->getMean();
                Eigen::Matrix3d cov = cell->getCov();
...
///////////////////////////////////////////////////////////////////////////
        ...
        if (stage1 && (f <= ftest1) && (dg >= perception_oru::NDTMatcherD2D::MoreThuente::min(ftol, gtol) * dginit))
        {
            stage1 = false;
        }
        ...
        if (stage1 && (f <= fx) && (f > ftest1))
        {
            ...
            infoc = perception_oru::NDTMatcherD2D::MoreThuente::cstep(stx,fxm,dgxm,sty,fym,dgym,stp,fm,dgm,
                                       brackt,stmin,stmax);
        ...
        else
        {
            ...
            infoc = perception_oru::NDTMatcherD2D::MoreThuente::cstep(stx,fx,dgx,sty,fy,dgy,stp,f,dg,
                                       brackt,stmin,stmax);
...
inline  bool matchFusion( perception_oru::NDTMap& targetNDT,
		    perception_oru::NDTMap& sourceNDT,
		    perception_oru::NDTMap& targetNDT_feat,
		    perception_oru::NDTMap& sourceNDT_feat,
		    ...
		    bool useInitialGuess, bool useNDT, bool useFeat, bool step_control, int ITR_MAX = 30, int n_neighbours = 2, double DELTA_SCORE = 10e-4, bool useSoftConstraints = true, bool step_control_fusion = true, bool useTikhonovRegularization = false)
{
  ...
  perception_oru::NDTMatcherD2D matcher_d2d;
  perception_oru::NDTMatcherFeatureD2D matcher_feat_d2d(corr_feat);
  ...
  matcher_d2d.n_neighbours = n_neighbours;
    ...
    std::vector<perception_oru::NDTCell*> nextNDT = sourceNDT.pseudoTransformNDT(T);
    std::vector<perception_oru::NDTCell*> nextNDT_feat = sourceNDT_feat.pseudoTransformNDT(T);
...
inline  bool matchFusion2d( perception_oru::NDTMap& targetNDT,
                      perception_oru::NDTMap& sourceNDT,
                      perception_oru::NDTMap& targetNDT_feat,
                      perception_oru::NDTMap& sourceNDT_feat,
                      const std::vector<std::pair<int, int> > &corr_feat,
                      ...
                      bool useInitialGuess, bool useNDT, bool useFeat, bool step_control, int ITR_MAX = 30, int n_neighbours = 2, double DELTA_SCORE = 10e-4)
  {
    ...
    perception_oru::NDTMatcherD2D_2D matcher_d2d_2d;
