...
void NDTGraphDisplay::updateHistoryLength()
{
  ...
  void NDTGraphDisplay::processMessage(const ndt_feature::NDTGraphMsg::ConstPtr& msg ){
    ...
    for(int i = 0 ; i < msg->nodes.size() ; ++i){
		...
		std::cout << "For map " << i << " there is " << msg->nodes[i].fuser.map.cells.size() << " cells " << std::endl;
		...
		for(int itr=0;itr<msg->nodes[i].fuser.map.cells.size();itr++){
			
			if(msg->nodes[i].fuser.map.cells[itr].hasGaussian_ == true){
				...
				if(!(msg->nodes[i].fuser.map.x_cell_size==msg->nodes[i].fuser.map.y_cell_size&&msg->nodes[i].fuser.map.y_cell_size==msg->nodes[i].fuser.map.z_cell_size)){ 
					ROS_ERROR("SOMETHING HAS GONE VERY WRONG YOUR VOXELL IS NOT A CUBE"); 
					//return false;
				}
				
				visual->setCell(msg->nodes[i].fuser.map.cells[itr],msg->nodes[i].fuser.map.x_cell_size);
