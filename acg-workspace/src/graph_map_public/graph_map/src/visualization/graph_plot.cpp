...
//  cout<<"graph_plot: initialize"<<endl;
  ...
  *observationPublisher_=nh_->advertise<visualization_msgs::MarkerArray>(OBSERVATIONS_TOPIC,10);
  *trajectoryPublisher_=nh_->advertise<geometry_msgs::PoseArray>(TRAJECTORY_TOPIC,10);
  initialized_=true;
}
void GraphPlot::PlotPoseGraph(const GraphMapPtr& graph){
  if(!initialized_)
    Initialize();
  //Publish poses
  ...
  for(int i=0;i<graph->Size();i++){
    ...
    ROS_DEBUG_STREAM( "pose_tmp " << pose_tmp.matrix() );
  ...
  os << std::setw(3)<< "(x,y)=("<<marker.pose.position.x<<","<<marker.pose.position.y<<")"<<std::endl;
...
void GraphPlot::PlotObservationVector(GraphMapPtr graph_map){
  ...
  if( GraphMapNavigatorPtr graph=boost::dynamic_pointer_cast<GraphMapNavigator>(graph_map)){
    for(int i=0;i<graph_map->Size();i++){ //step through graph and mark all color
    ...
    std::vector<MapNodePtr> closest_nodes= graph->GetClosestNodes(graph_map->GetCurrentNodePose(),DBL_MAX,true);
    ...
    for(int i=id;i<max_id;i++){ // remove already plotted unit which are not overwrited
...
void GraphPlot::PublishMapAsOccupancy(mean_vector &mean , const std::vector<double> &occupancy, double scale, const Eigen::Affine3d &offset, std::string ns, int id){
  ...
  for(unsigned int i=0;i<mean.size();i++){
    ...
    p.y=m_tmp(1);
    p.z=m_tmp(2);
    std::cerr<<"MISSING ndt_visualisation::assignColor(marker, 3);"<<std::endl;
...
void GraphPlot::sendMapToRviz(mean_vector &mean, cov_vector &cov, ros::Publisher *mapPublisher, std::string frame, int color,const Affine3d &offset, std::string ns,int markerType){
...
void GraphPlot::GetAllCellsMeanCov(const perception_oru::NDTMap *mapPtr,cov_vector &cov,mean_vector &mean, std::vector<double> &occ){
  cov.clear();
  mean.clear();
...
void GraphPlot::GetAllCellsMeanCov( std::vector<perception_oru::NDTCell*>cells,cov_vector &cov,mean_vector &mean, std::vector<double> &occupancy){
  cov.clear();
  mean.clear();
  for(int i=0;i<cells.size();i++){
    cov.push_back(cells[i]->getCov());
...
void GraphPlot::GetAllCellsOccupancy( std::vector<perception_oru::NDTCell*>cells,mean_vector &center_position, std::vector<double> &occupancy){
