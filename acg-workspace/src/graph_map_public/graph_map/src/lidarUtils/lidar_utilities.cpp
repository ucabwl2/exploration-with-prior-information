...
void ScanPlot::plot(const sensor_msgs::LaserScan &scan,const scanRepresentation plot_type,const std::string PlotOptsX,const std::string PlotOptsY,double alpha){
  ...
  std::cout<<"derivative size:="<<der.size()<<", data size="<<data_set.size()<<std::endl;
...
void ScanPlot::derivative(pair_vector_double &data_in, pair_vector_double &derivative){
  ...
  for(int i=1;i<data_in.size()-1;i++){
    ...
    std::cout<<data_in[i].first<<std::endl;
  }
