...
//	    std::cout << "Getting the center" << std::endl;
        pcl::PointXYZ point = cell_to_copy.getCenter();
...
//	        std::cout << "copying the cell" << std::endl;
            ...
            if(a != cellSizeX){
                throw std::runtime_error("Dimension of cells in lazygrid does not correspond to the one being copied along x.");
            }
            if(b != cellSizeY){
                throw std::runtime_error("Dimension of cells in lazygrid does not correspond to the one being copied along y.");
            }
            if(c != cellSizeZ){
                throw std::runtime_error("Dimension of cells in lazygrid does not correspond to the one being copied along z.");
            }
...
bool LazyGrid::traceLineWithEndpoint(const Eigen::Vector3d &origin, const Eigen::Vector3d &endpoint, const double& maxz, std::set<NDTCell*> &cells, Eigen::Vector3d &final_point)
{
    if(endpoint[2]>maxz)
    {
    ...
    for(int i=0; i<N-2; i++)
    {
        ...
        if(idx < sizeX && idy < sizeY && idz < sizeZ && idx >=0 && idy >=0 && idz >=0){
            ...
            if(ptCell !=NULL) {
                ...
                this->addPoint(pt); ///Add fake point to initialize! Why is the cell not added to the vector ? Should use a set also
            }
        } else {
