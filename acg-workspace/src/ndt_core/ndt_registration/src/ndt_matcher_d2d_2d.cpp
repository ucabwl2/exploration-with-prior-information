                            ...
                            bool useDefaultGridResolutions, std::vector<double> _resolutions)
{
  ...
  lfd1 = 1.0; //lfd1/(double)sourceNDT.getMyIndex()->size(); //current_resolution*2.5; //Changed november 2017 from 1.0
                              ...
                              bool useInitialGuess)
{
  ...
  while(!convergence)
  {
    ...
    if (score_gradient_2d.norm()<= 10e-2*DELTA_SCORE){
      std::cout<<"\%gradient vanished at itr: "<<itr_ctr<<", norm : " << score_gradient_2d.norm() << std::endl;
